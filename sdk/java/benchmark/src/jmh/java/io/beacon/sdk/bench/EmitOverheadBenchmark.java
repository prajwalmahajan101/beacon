 ...
 *
 * <p>Workload: a precomputed LogRecord with a 16-byte body and 4 string attributes; no MDC; no live
 * OTel Span. The sink is BatchSink.NOOP, so the pipeline measured is enricher.enrich →
 * redactor.redact → buffer.offer. Network I/O, OTel serialization, and the flusher thread are all
 * out of scope (the flusher runs asynchronously on a daemon thread and never blocks emit).
 *
 * <p>To reproduce locally: ./gradlew :beacon-sdk-java-benchmark:jmh To reproduce in CI mode
 * (faster, lower-resolution): ./gradlew :beacon-sdk-java-benchmark:jmh -PbenchmarkCI
 *
...
public class EmitOverheadBenchmark {
  ...
  public void setUp() {
    ...
    Map<String, Object> attrs = Map.<String, Object>of("a", "1", "b", "2", "c", "3", "d", "4");
    record =
        LogRecord.builder()
