...
/**
 * Spring {@code @ConfigurationProperties} POJO mapping the 13 canonical {@code beacon.*} surfaces —
 * JSDK-07 / M1.7 Plan 02-02 — onto the M1.6-frozen {@link io.beacon.sdk.config.BeaconConfig} record
 * (which carries 15 internal components).
 *
 * <h2>13 canonical surfaces</h2>
 *
 * <ol>
 *   <li>{@code beacon.endpoint} — OTLP gRPC endpoint.
 *   <li>{@code beacon.api-key} — bearer/API key.
 *   <li>{@code beacon.buffer-capacity} — bounded buffer capacity (default 10_000).
 *   <li>{@code beacon.drop-policy} — {@code DROP_OLDEST | DROP_NEWEST | SPILL_FALLBACK}.
 *   <li>{@code beacon.batch-max-records} — max records per OTLP batch (default 512).
 *   <li>{@code beacon.flush-interval-ms} — time-based flush trigger (default 1_000).
 *   <li>{@code beacon.max-retries} — retry attempts on retriable OTLP failures (default 5).
 *   <li>{@code beacon.backoff-base-ms} — exponential-backoff base (default 100).
 *   <li>{@code beacon.backoff-max-ms} — exponential-backoff cap (default 5_000).
 *   <li>{@code beacon.fallback-sink} — {@code stderr | file:/path} (default {@code stderr}).
 *   <li>{@code beacon.shutdown-drain-timeout-ms} — C9 drain timeout (default 5_000).
 *   <li>{@code beacon.sampling-ratio} — head-sampling ratio 0.0-1.0 (default 1.0).
 *   <li>{@code beacon.redact} — composite. Nested:
 *       <ul>
 *         <li>{@code keys} — user PII keys to scrub.
 *         <li>{@code defaults} — union with baseline {@code
 *             password|authorization|api_key|secret|token}.
 *         <li>{@code timeout-ms} — per-record redaction budget; maps to internal {@code
 *             BeaconConfig.redactorTimeoutMs} slot (folded from M1.6 top-level key per ADR-0009 §3
 *             Option-A).
 *       </ul>
 * </ol>
 *
...
public class BeaconProperties {
  ...
  public BeaconConfig.DropPolicy getDropPolicy() {
    return dropPolicy;
  }

  public void setDropPolicy(BeaconConfig.DropPolicy dropPolicy) {
  ...
  public String getFallbackSink() {
    return fallbackSink;
  }

  public void setFallbackSink(String fallbackSink) {
  ...
  /**
   * Construct a {@link BeaconConfig} from these properties. Maps {@code redact.timeoutMs} into the
   * internal {@code redactorTimeoutMs} slot (ADR-0009 §3 Option-A fold). The 15-component
   * constructor order MUST mirror the frozen record order — see {@link BeaconConfig}.
   */
  ...
  /**
   * Nested composite for canonical surface 13 ({@code beacon.redact}). Holds {@code keys}, {@code
   * defaults}, and {@code timeoutMs} (binds {@code beacon.redact.timeout-ms}; maps to {@link
   * BeaconConfig#redactorTimeoutMs()}).
   */
  public static class Redact {
    ...
    /**
     * If {@code true}, union user keys with the always-on baseline {@code
     * password|authorization|api_key|secret|token}. Default {@code true}.
     */
    private boolean defaults = true;

    /**
     * Per-record redaction budget on the caller thread. Binds {@code beacon.redact.timeout-ms};
     * maps to {@link BeaconConfig#redactorTimeoutMs()}. Default {@code 5L} (matches {@link
