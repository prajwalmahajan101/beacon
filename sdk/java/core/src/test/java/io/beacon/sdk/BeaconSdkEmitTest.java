...
class BeaconSdkEmitTest {
  ...
  void emit_enqueues_records_under_capacity_and_tracks_metrics() {
    ...
    // Stop the flusher (non-draining) so we observe pure buffer/metrics
    // behaviour. M1.5: BeaconSdk.close() drains, so we use the flusher's
    // direct stop() instead. End-to-end flush coverage lives in
    // BatchFlusherTest + C4/C5.
    sdk.flusher().stop();
  ...
  void emit_with_drop_oldest_keeps_size_at_capacity_and_drops_excess() {
    BeaconConfig cfg =
        BeaconConfig.defaults().withBufferCapacity(10).withDropPolicy(DropPolicy.DROP_OLDEST);
