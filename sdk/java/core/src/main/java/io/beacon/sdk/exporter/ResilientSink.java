 ...
 *
 * <p>On {@link #accept(List)}: invoke the delegate; if it throws, sleep for {@link
 * RetryPolicy#nextDelayMs(int)} and try again, up to {@code maxRetries} retries (i.e. {@code
 * maxRetries + 1} total attempts). On final exhaustion the batch is handed to {@link FallbackSink}
 * — never silently dropped.
 *
 * <p>Metric semantics:
 *
 * <ul>
 *   <li>{@code exported += batch.size()} on the first successful delegate call.
 *   <li>{@code export_failures} increments once per failed delegate attempt.
 *   <li>{@code fallback_writes += batch.size()} fires from inside {@code FallbackSink}.
 * </ul>
 *
 * <p>Threading: this sink is called on the {@code BatchFlusher} daemon thread. {@code Thread.sleep}
 * blocks that thread for up to {@code maxRetries * backoffMaxMs}; the bounded buffer's drop policy
 * provides back-pressure during that window.
 */
public final class ResilientSink implements BatchSink {
  ...
  public ResilientSink(
      BatchSink delegate, RetryPolicy retryPolicy, FallbackSink fallback, SdkMetrics metrics) {
  ...
  public static ResilientSink of(BatchSink delegate, BeaconConfig config, SdkMetrics metrics) {
    ...
    RetryPolicy rp =
        new RetryPolicy(config.maxRetries(), config.backoffBaseMs(), config.backoffMaxMs());
