...
/**
 * Unit coverage for {@link BeaconExecutors} — OTel Context + SLF4J MDC capture/restore
 * across executor boundaries. Every test shuts the per-test executor down in @AfterEach
 * and clears MDC, so a misbehaving wrap doesn't pollute neighbouring tests.
 */
class BeaconExecutorsTest {
    ...
    void runnable_carries_mdc_to_worker_thread() throws Exception {
        ...
        Runnable wrappedRun = BeaconExecutors.wrap(() -> {
    ...
    void runnable_restores_prior_mdc_on_worker() throws Exception {
        ...
        MDC.put("key", "caller");
        Runnable wrapped = BeaconExecutors.wrap(() -> {
    ...
    void runnable_clears_mdc_when_caller_had_none() throws Exception {
        ...
        executor.submit(() -> {
            ...
            // Now run the wrapped body — caller snapshot was null, so MDC inside body must be cleared.
            wrapped.run();
    ...
    void otel_context_propagates_via_wrap() throws Exception {
        ...
        try (Scope ignored = Span.wrap(sc).makeCurrent()) {
            Callable<Void> task = BeaconExecutors.wrap(() -> {
    ...
    void exception_in_task_still_restores_mdc() throws Exception {
        ...
        MDC.put("key", "caller");
        Runnable boom = BeaconExecutors.wrap((Runnable) () -> { throw new RuntimeException("boom"); });

        AtomicReference<String> afterFailure = new AtomicReference<>();
        Future<?> f = executor.submit(() -> {
            try { boom.run(); } catch (RuntimeException expected) { /* ignore */ }
