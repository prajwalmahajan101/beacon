 ...
 * (beacon-s0-contract/spec/severity-table.json). Pitfall #4 — cross-SDK severity-table
 * divergence guard, Java side. M2 (Python SDK) will mirror this test against the same
 * artifact.
 */
class SeverityMapperContractTest {

    private static final Path CONTRACT_JSON = Paths.get(
            "..", "beacon-s0-contract", "spec", "severity-table.json")
            .toAbsolutePath().normalize();

    @Test
    @DisplayName("contract: severity-table.json defines exactly 6 bands with anchors [1,5,9,13,17,21]")
    void sixBandsWithSpecAnchors() throws Exception {
        ...
        for (int i = 0; i < 6; i++) {
            assertThat(bands.get(i).get("anchor").asInt())
                    .as("band[%d].anchor", i).isEqualTo(expected[i]);
        }
    }
    ...
    void numberForReturnsContractAnchor() throws Exception {
        ...
        for (JsonNode band : bands) {
            ...
            int anchor = band.get("anchor").asInt();
            assertThat(SeverityMapper.numberFor(name))
                    .as("numberFor(%s)", name).isEqualTo(anchor);
        }
    }
