 ...
 * {@link #env_wins_over_sysprop_and_builder_when_present()} which is annotated with
 * {@link EnabledIfEnvironmentVariable} and is skipped unless {@code BEACON_REDACT_KEYS} is set
 * externally (e.g. in CI). All other precedence layers (sysprop, builder) are exercised on every
 * run.
 */
class BeaconConfigLoaderTest {
    ...
    void effectiveRedactKeys_lowercases_under_Locale_ROOT() {
        ...
        Set<String> eff = BeaconConfigLoader.effectiveRedactKeys(List.of("PASSWORD", "X-Api-Key"), false);
