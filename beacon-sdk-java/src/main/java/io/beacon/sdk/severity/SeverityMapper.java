 ...
 *
 * <p>The band table is loaded from {@code beacon-s0-contract/spec/severity-table.json}
 * at class init (M1.8 Plan 03-02 — cross-SDK contract artifact). Public API
 * (numberFor / textFor / bandFor) is unchanged from M1.1; only the data source moved.
 *
 * <p>Resolution order for the artifact:
 * <ol>
 *   <li>Classpath resource {@code /beacon-s0-contract/spec/severity-table.json}
 *       (works in test runs where the contract dir is added to the test resources).</li>
 *   <li>Filesystem relative paths walked from common CWDs (conformance harness vs SDK
 *       gradle vs project root vs IDE run).</li>
 * </ol>
 *
 * <p>If none resolve, the class init throws — fail-fast is the correct behaviour;
 * the SDK is unusable without a valid band table.
 */
public final class SeverityMapper {
    ...
    public enum Band {
        ...
        public int rangeMin() { return rangeMin; }
        public int rangeMax() { return rangeMax; }
