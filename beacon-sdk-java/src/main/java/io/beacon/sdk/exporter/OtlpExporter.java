...
/**
 * Production OTLP transport — wraps OTel Java's {@link OtlpGrpcLogRecordExporter} or
 * {@link OtlpHttpLogRecordExporter} as a {@link BatchSink}. Implements spec/02 §2.1's
 * directive that SDKs build on top of OpenTelemetry rather than reimplementing the wire.
 *
 * <p>Conversion is done via {@link SdkLoggerProvider} + {@link Logger}, which lets OTel
 * own the {@code LogRecordData} model. Each Beacon {@link LogRecord} maps to one OTel
 * log record (timestamp, severity number + text, body, attributes). Trace context
 * propagation lands in M1.6 (C11); resource conversion is best-effort flat-attribute
 * for now and tightens in M1.7 when the Spring Boot starter wires Resource detection.</p>
 *
 * <p>Resilience (retry/backoff + fallback) sits OUTSIDE this class — production wiring
 * is {@code ResilientSink.of(new OtlpExporter(...), config, metrics)}. This class fails
 * fast by throwing a {@link RuntimeException} when the OTLP forceFlush returns an error,
 * so {@code ResilientSink}'s retry loop drives the behaviour spec demands.</p>
 */
public final class OtlpExporter implements BatchSink, AutoCloseable {
    ...
    public OtlpExporter(String endpoint, Transport transport) {
        ...
        LogRecordExporter otelExporter = switch (transport) {
    ...
    public void accept(List<LogRecord> batch) {
        for (LogRecord r : batch) {
            ...
            if (r.timestamp() != null) {
                long epochNanos = r.timestamp().getEpochSecond() * 1_000_000_000L
                        + r.timestamp().getNano();
