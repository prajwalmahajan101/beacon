 ...
 *
 * <ol>
 *   <li>If {@code Span.current().getSpanContext()} is valid → those ids win.
 *   <li>Else fall back to SLF4J {@code MDC.get("trace_id")} / {@code MDC.get("span_id")}, accepted
 *       only when they match the OTel/W3C hex shape (32-hex traceId, 16-hex spanId).
 *   <li>Else leave both fields {@code null} — never write zero-hex placeholders, never fabricate.
 * </ol>
 *
 * <p>Read-only with respect to OTel Context: the enricher never starts a Span and never calls into
 * {@code Tracer}. Pre-stamped records (test injection) are honored — input values win over both
 * sources.
 *
...
public final class Enricher {
  ...
  private static LogRecord stamp(LogRecord in, String traceId, String spanId) {
    ...
    return LogRecord.Builder.from(in).traceId(effectiveTrace).spanId(effectiveSpan).build();
  }
