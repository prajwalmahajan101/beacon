 ...
 *
 * <p>Backed by {@link ArrayBlockingQueue}. Capacity is fixed at construction; {@link
 * #offer(LogRecord)} never blocks (spec/02 §2.1) and applies the configured {@link DropPolicy} when
 * full (spec/02 §2.2).
 */
public final class BoundedBuffer {
  ...
  /**
   * Enqueue a record without blocking. Returns {@code true} if the record was accepted into the
   * buffer, {@code false} if the policy was {@code DROP_NEWEST} and the buffer was full.
   *
   * <p>{@code DROP_OLDEST} always returns {@code true} — it evicts the head and accepts the new
   * record, incrementing the dropped counter per eviction.
   */
  public boolean offer(LogRecord record) {
    ...
    switch (policy) {
      ...
      case SPILL_FALLBACK ->
          throw new UnsupportedOperationException("M1.4: SPILL_FALLBACK requires FallbackSink");
  ...
  /**
   * Wait up to {@code timeoutMs} for a record and return it, or {@code null} if the timeout elapsed
   * without one. Lets the batch flusher block efficiently instead of busy-polling. A negative or
