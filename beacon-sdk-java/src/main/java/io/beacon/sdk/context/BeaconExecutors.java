...
/**
 * Public helper for propagating Beacon-relevant context (OTel {@link Context} + SLF4J
 * {@link MDC}) across executor boundaries.
 *
 * <p>Use {@link #wrap(Executor)} / {@link #wrap(ExecutorService)} for executors you
 * control, or {@link #wrap(Runnable)} / {@link #wrap(Callable)} for ad-hoc
 * {@code CompletableFuture.supplyAsync(BeaconExecutors.wrap(...), executor)} calls.
 *
 * <p>OTel half: delegates to {@link Context#taskWrapping(Executor)} — the official,
 * tested propagation API. MDC half: a {@code Map<String,String>} snapshot taken at
 * submission and restored on execution, with prior worker-thread MDC state restored
 * in a {@code finally} block.
 *
 * <p>See ADR-0008 ({@code docs/adr/0008-async-context-propagation.md}) for the rationale.
 *
 * <p><strong>Misuse:</strong> submitting work to a non-wrapped executor silently loses
 * context. Wrap the executor (or wrap the {@link Runnable}/{@link Callable}) at the
...
public final class BeaconExecutors {
    ...
    /**
     * Wrap an {@link Executor} so every submitted {@link Runnable} carries the
     * caller-thread OTel Context + MDC at the moment of {@code execute(...)}.
     */
    ...
    /**
     * Wrap an {@link ExecutorService} so every submitted {@link Runnable}/{@link Callable}
     * carries the caller-thread OTel Context + MDC. Lifecycle methods pass through.
     */
    ...
    /**
     * Wrap a {@link Runnable} so it carries the caller-thread OTel Context + MDC when
     * it runs on another thread (e.g. via {@code CompletableFuture.runAsync(wrap(r), executor)}).
     */
    ...
    /**
     * Wrap a {@link Callable} so it carries the caller-thread OTel Context + MDC when
     * it runs on another thread (e.g. via {@code CompletableFuture.supplyAsync(wrap(c), executor)}).
     */
    ...
    /**
     * Thin wrapper that decorates every submitted task with a per-submission MDC snapshot.
     * The OTel-context half is already applied by {@code delegate} (returned from
     * {@link Context#taskWrapping(ExecutorService)} — see {@link #wrap(ExecutorService)}).
     */
