...
public record BeaconConfig(
    ...
    public BeaconConfig withBufferCapacity(int n) {
        return new BeaconConfig(endpoint, apiKey, n, dropPolicy, batchMaxRecords, flushIntervalMs,
                ...
                redactKeys, samplingRatio, redactorTimeoutMs, redactDefaults);
    }
    ...
    public BeaconConfig withRedactKeys(List<String> keys) {
        return new BeaconConfig(endpoint, apiKey, bufferCapacity, dropPolicy, batchMaxRecords, flushIntervalMs,
                maxRetries, backoffBaseMs, backoffMaxMs, fallbackSink, shutdownDrainTimeoutMs,
                keys, samplingRatio, redactorTimeoutMs, redactDefaults);
    }

    public BeaconConfig withRedactorTimeoutMs(long ms) {
        return new BeaconConfig(endpoint, apiKey, bufferCapacity, dropPolicy, batchMaxRecords, flushIntervalMs,
                maxRetries, backoffBaseMs, backoffMaxMs, fallbackSink, shutdownDrainTimeoutMs,
                redactKeys, samplingRatio, ms, redactDefaults);
    }

    public BeaconConfig withRedactDefaults(boolean enabled) {
        return new BeaconConfig(endpoint, apiKey, bufferCapacity, dropPolicy, batchMaxRecords, flushIntervalMs,
                maxRetries, backoffBaseMs, backoffMaxMs, fallbackSink, shutdownDrainTimeoutMs,
                redactKeys, samplingRatio, redactorTimeoutMs, enabled);
    }
}
