...
- M3.0d: `platform/gateway/build.gradle.kts` — a `:beacon-gateway:e2eTest` task (runs only `@Tag("e2e")`, excluded from the default `test` so `gateway.yml` stays fast) decoupled from the root JaCoCo convention in both directions; and a **test-classpath-only** OTel realignment (`eachDependency` → the SDK's compiled 1.42.0) because Spring Boot dependency management otherwise downgrades `exporter-otlp`/`sdk-logs` to 1.37.0 while `opentelemetry-api-incubator` (AnyValue) floats to 1.44.1-alpha, `NoClassDefFoundError`-ing the SDK's real `OtlpExporter` (a latent SDK runtime-dep gap surfaced by the first real-exporter exercise).
