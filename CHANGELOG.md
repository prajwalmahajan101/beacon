...
- M3.0d: **full-stack ingest E2E** driving BOTH real SDKs end-to-end (`SDK.emit → OTLP → gateway → Kafka → Vector → Elasticsearch`, asserted searchable in `beacon-logs`). Java: `IngestE2ETest` (`@Tag("e2e")`, a segregated `:beacon-gateway:e2eTest` task) constructs the real `BeaconSdk` (`OtlpExporter` → `ResilientSink`). Python: a new `platform/e2e/` uv project (path-dep on `sdk/python/core`) whose pytest drives `build_emit_pipeline` → `BeaconLoggingHandler` → `drain_and_stop`. Both boot the **real `docker-compose.yml`** via Testcontainers (local-compose mode) and cover **two scenarios each** — direct to the gateway and through the OTel Collector — closing **INGEST-16** (and functionally demonstrating **INGEST-12**). Export is forced with an explicit drain and asserted via ES (never the SDK return, PITFALLS #29).
- M3.0d: **OTel-Collector-fronted path** — `deploy/collector/collector.yaml` (core collector: `otlp` receiver → `otlp_grpc` exporter to `gateway:4317`) + `docker-compose.collector.yml` overlay adding the `otel-collector` service (pinned `otel/opentelemetry-collector:0.156.0`, host 5317/5318). The gateway stays the single Kafka producer; the Collector is a transparent proxy in front (PRD §13.1, ADR-0025).
- M3.0d: `.github/workflows/ingest.yml` — a dedicated CI gate that builds the gateway image once, then runs the Java (`:beacon-gateway:e2eTest`) and Python (`platform/e2e`) E2Es on any ingest-path change; uploads JUnit + pytest reports.
- M3.0c: the real Vector **indexer** pipeline — `deploy/vector/vector.yaml` replaces the M3.0a `demo_logs → blackhole` placeholder with a `kafka` source (`kafka:29092`, topic `beacon.logs`, `group_id: beacon-indexer`, `auto_offset_reset: earliest`, `decoding.codec: json`, **`log_namespace: true`**) → `elasticsearch` sink (`http://elasticsearch:9200`, `api_version: v8`, bulk into the plain **`beacon-logs`** index, sink healthcheck off). `log_namespace: true` keeps the decoded M0 record as the event root and routes Kafka metadata to the metadata namespace, so ES receives a clean 12-key M0 doc (verified — no `%kafka` pollution, no msg-timestamp vs record-`timestamp` collision). Skeleton by design: plain index + ES dynamic mapping; index template + ILM are M3.3, DLQ/idempotency/composite key are M3.1, the full-SDK Testcontainers E2E + `ingest.yml` CI gate are M3.0d (5.3). No ADR — mechanical Vector config.
- M3.0c: `scripts/smoke-indexer.sh` — a committed end-to-end smoke test that brings up kafka+es+vector (`--wait`), seeds one canonical M0 record (compacted from `contract/schema/examples/log-valid.json`) onto `beacon.logs` via the broker console producer, polls ES `_search` until the doc is searchable (60s timeout), and asserts the indexed body. Exits non-zero on failure (CI-adoptable by 5.3); `SMOKE_KEEP=1` leaves the stack up.
...
- M3.0d: `docker-compose.yml` / `docker-compose.collector.yml` — removed `container_name` from every service (Testcontainers' `ComposeContainer` rejects it at parse time, even in local-compose mode; nothing operational depended on the fixed names — inter-service comms + healthchecks use service names / `localhost`). Gave the gateway a stable `image: beacon-gateway:local` tag so a pre-built image is reused across the E2E's random compose project names instead of rebuilding the in-Docker Gradle `bootJar` each run.
- M3.0d: `platform/gateway/build.gradle.kts` — a `:beacon-gateway:e2eTest` task (runs only `@Tag("e2e")`, excluded from the default `test` so `gateway.yml` stays fast) decoupled from the root JaCoCo convention in both directions; and the OpenTelemetry BOM imported via Spring's `dependencyManagement` so Boot's dependency management no longer downgrades the stable OTel artifacts on this module (see the Fixed entry).
- M3.0c: `docker-compose.yml` — the `vector:` service now `depends_on` **kafka + elasticsearch** `service_healthy` (it runs the real indexer, so it starts after both ends of the pipeline are up rather than racing an absent broker/ES on boot); the stale "placeholder" wording in the SCOPE header and the vector service comment was refreshed to describe the real pipeline.
...
- M3.0d: **SDK OpenTelemetry surface could `NoClassDefFoundError` the real OTLP exporter.** `OtlpExporter.accept()` uses `AnyValue` from `opentelemetry-api-incubator`, which is pulled only transitively (via the OTLP logs exporter + the logback-appender instrumentation) and floated to a newer alpha (1.44.1) than the SDK's 1.42.0 `sdk-logs`/`exporter-otlp` line — and under Spring Boot dependency management the stable artifacts were *also* downgraded to Boot's 1.37.0. Either skew `NoClassDefFoundError`s at emit. Latent because the SDK's own tests never exercise `accept()`; surfaced by the M3.0d E2E (the first real-exporter run). Fixed at the root: `sdk/java/core` adds a strict dependency constraint pinning `opentelemetry-api-incubator` to the SDK's OTel line (propagates to `beacon-sdk-spring-adapter` and `:beacon-gateway`), and `:beacon-gateway` imports the OTel BOM through Spring's `dependencyManagement` (which overrides Boot's managed versions) so the stable artifacts track 1.42.0 too. Verified: `exporter-otlp`/`sdk-logs` = 1.42.0 and `api-incubator` = 1.42.0-alpha across all three modules; `:beacon-gateway:e2eTest` green with no per-classpath workaround.

### Verified

- M3.0d: `./gradlew :beacon-gateway:e2eTest` green (2/2 — Java direct + via-Collector) and `cd platform/e2e && uv run pytest` green (2/2 — Python direct + via-Collector) on Docker Engine 29.5.1: each real SDK's emit was searchable in the `beacon-logs` ES index within the timeout. Manual overlay stand-up `docker compose -f docker-compose.yml -f docker-compose.collector.yml up -d --wait` brought all five services (kafka, elasticsearch, vector, gateway, otel-collector) healthy; a raw OTLP record through the Collector (`localhost:5318`) landed in ES; `down -v` clean. Default `:beacon-gateway:test` unchanged (e2e tag excluded).
- M3.0c: `./scripts/smoke-indexer.sh` green on Docker Engine 29.5.1 — kafka+es+vector came up healthy, a canonical M0 record seeded onto `beacon.logs` was returned by `GET /beacon-logs/_search` within the timeout (hits=1, `body` match). The indexed `_source` is exactly the 12 M0 keys — no Kafka-metadata pollution, confirming `log_namespace: true`. ES dynamic-mapped `timestamp`/`observed_timestamp` as `date` and accepted the RFC3339-ns string (stored at ms precision — nanos-preserving mapping is M3.3); no ES-side bulk rejections. `docker compose down -v` clean. `vector validate` passes on the config.
