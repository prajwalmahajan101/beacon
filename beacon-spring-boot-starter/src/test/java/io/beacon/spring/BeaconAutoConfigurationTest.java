...
/**
 * Wires {@link BeaconAutoConfiguration} through {@link ApplicationContextRunner} and asserts the
 * four contributed beans, the {@code beacon.enabled=false} opt-out gate, and
 * {@code @ConditionalOnMissingBean} precedence for user overrides. Also asserts the {@link
 * BeaconLogbackAppender} is programmatically attached to the root Logback logger (Pitfall #18 — no
 * {@code logback-spring.xml} mutation).
 */
class BeaconAutoConfigurationTest {
  ...
  void wiringRegistersBeaconSdkBean() {
    runner
        .withPropertyValues("beacon.endpoint=http://localhost:4317")
        .run(
            ctx -> {
    ...
    runner
        ...
        .run(
            ctx -> {
              ...
              BeaconTaskDecorator decorator =
                  ctx.getBean("beaconTaskDecorator", BeaconTaskDecorator.class);
              Runnable raw =
                  () -> {
                    /* no-op */
                  };
  ...
  void beaconDisabledSkipsAllBeans() {
    runner
        .withPropertyValues("beacon.enabled=false", "beacon.endpoint=http://localhost:4317")
        .run(
            ctx -> {
    ...
    BeaconSdk userBeaconSdk() {
      ...
      BeaconConfig override =
          new BeaconConfig(
              ...
              cfg.backoffMaxMs(),
              cfg.fallbackSink(),
              cfg.shutdownDrainTimeoutMs(),
              cfg.redactKeys(),
              cfg.samplingRatio(),
              cfg.redactorTimeoutMs(),
              cfg.redactDefaults());
