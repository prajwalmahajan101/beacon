...
/**
 * Binds the 13 canonical {@code beacon.*} surfaces from {@code application.yml}-style
 * property sources through {@link BeaconProperties#toBeaconConfig()} into the
 * frozen 15-component {@link BeaconConfig} record. Includes the regression guard for
 * the ADR-0009 §3 Option-A fold (deprecated top-level {@code beacon.redactor-timeout-ms}
 * must NOT bind).
 *
 * <p>Uses {@link ApplicationContextRunner} so the test exercises Spring Boot's real
 * relaxed-binding machinery (kebab-case → camelCase, nested composite) without
...
class BeaconPropertiesBindingTest {
    ...
    /**
     * Regression guard for ADR-0009 §3 Option-A: the legacy top-level key
     * {@code beacon.redactor-timeout-ms} (M1.6's pre-fold surface) must NOT bind.
     * The starter exposes the value only via the nested {@code beacon.redact.timeout-ms}.
     */
    @Test
    void rejectsTopLevelRedactorTimeoutMsKey() {
        runner.withPropertyValues(
            ...
            BeaconConfig c = p.toBeaconConfig();
            assertThat(c.redactorTimeoutMs())
                    .as("deprecated top-level beacon.redactor-timeout-ms must not bind; "
